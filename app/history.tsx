...
export default function HistoryScreen() {
  ...
  const loadHistory = useCallback(async () => {
    ...
    const dates = Array.from(
      new Set<ISODateString>([...taskDates, ...reviews.map((review) => review.date)]),
    ).sort((a, b) => b.localeCompare(a));

    const nextSummaries = await Promise.all(
      dates.map(async (date) => {
        const [tasks, focusSessions, proofs] = await Promise.all([
          getTasksByDate(date),
        ...
        return {
          date,
          review: reviews.find((review) => review.date === date),
          stats: buildReviewStats(tasks, focusSessions, proofs),
        };
      }),
    );
      ...
      <ScrollView contentContainerStyle={styles.content}>
        <View style={styles.hero}>
          ...
          <Text style={styles.description}>
            每一天都留在这里：完成了什么、专注了多久，以及写给自己的那句话。
          </Text>
        </View>
        ...
        {summaries.length === 0 ? (
          <View style={styles.empty}>
            ...
            <Text style={styles.emptyTitle}>还没有历史记录，先认真过完今天。</Text>
            <Text style={styles.emptyText}>完成一次晚间复盘后，这里会出现每天的收尾记录。</Text>
          </View>
        ) : (
          summaries.map((summary, index) => (
            <Pressable
              ...
              key={summary.date}
              onPress={() => router.push(`/review/${summary.date}` as Href)}
              style={({ pressed }) => [styles.record, pressed ? styles.pressed : undefined]}
            >
              <View style={styles.recordHeader}>
                ...
                <View style={styles.recordHeaderCopy}>
                  ...
                  <Text style={styles.ratingText}>
                    {summary.review?.rating ? `${summary.review.rating} 分` : '未评分'}
                  </Text>
                </View>
              </View>
              ...
              {summary.stats.proofPhotos.length > 0 ? (
                <View style={styles.photoPreview}>
                  <ProofPhotoWall proofs={summary.stats.proofPhotos} maxPhotos={3} />
                </View>
              ) : null}
            </Pressable>
          ))
        ...
        <PrimaryButton onPress={() => router.push('/' as Href)} variant="soft">
          回到今日
        </PrimaryButton>
      </ScrollView>
    </SafeAreaView>
...
function Metric({ label, theme, value }: { label: string; theme: AppTheme; value: string }) {
  return (
    ...
    >
      <Text style={{ ...typography.caption, color: theme.primary, fontWeight: '900' }}>{value}</Text>
      <Text style={{ ...typography.micro, color: theme.textMuted, fontWeight: '900' }}>{label}</Text>
    </View>
  );
}
