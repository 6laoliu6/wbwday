...
import { ActivityIndicator, Alert, Image, KeyboardAvoidingView, Platform, SafeAreaView, ScrollView, StyleSheet, Text, View } from 'react-native';
...
type SelectedImage = { uri: string; isPersisted: boolean };
...
export default function ProofScreen() {
  ...
  const loadData = useCallback(async () => {
    ...
    const [nextTask, nextProof] = await Promise.all([getTaskById(taskId), getLatestCompletionProofByTaskId(taskId)]);
    setTask(nextTask);
  ...
  const takePhoto = useCallback(async () => {
    ...
    if (!permission.granted) { Alert.alert('需要相机权限才能拍照还愿。'); return; }
    const result = await ImagePicker.launchCameraAsync({ allowsEditing: false, mediaTypes: ['images'], quality: 0.9 });
  ...
  const pickImage = useCallback(async () => {
    ...
    if (!permission.granted) { Alert.alert('需要相册权限才能选择还愿照片。'); return; }
    const result = await ImagePicker.launchImageLibraryAsync({ allowsEditing: false, mediaTypes: ['images'], quality: 0.9 });
    if (result.canceled || !result.assets?.[0]?.uri) return;
    setSelectedImage({ uri: result.assets[0].uri, isPersisted: false });
    void hapticSelection();
  }, []);

  const removeSelectedImage = useCallback(() => { void hapticWarning(); setSelectedImage(undefined); }, []);

  const saveProof = useCallback(async (confirmedWithoutImage = false) => {
    if (!taskId || !task || saving) return;
    ...
    try {
      ...
      const proof = await saveCompletionProof(taskId, { imageUri, thumbnailUri, completionStatus, actualResult, note }, existingProof?.id);

      if (!selectedImage && existingProof?.imageUri) {
        await Promise.all([deleteProofImage(existingProof.imageUri), deleteProofImage(existingProof.thumbnailUri)]);
      }
      ...
      Alert.alert('这一刻，收好啦', '还愿已保存。', [
        { text: '好', onPress: () => router.replace(`/task/${proof.taskId}` as Href) },
      ]);
    } catch {
      void hapticError();
            ...
            <View style={styles.statusRow}>
              {completionStatusOptions.map((status) => (
                <ThemedChip
                  ...
                  tone={status === 'completed' ? 'success' : status === 'partial' ? 'warning' : 'primary'}
                />
              ))}
            </View>
          </View>

          <ThemedTextInput label="实际完成情况" multiline onChangeText={setActualResult} placeholder="例如：实际写了 1800 字" value={actualResult} />
          <ThemedTextInput label="写一句给今天的自己" multiline onChangeText={setNote} placeholder="例如：今天也有好好兑现。" value={note} inputStyle={styles.journalInput} />
          ...
          <View style={styles.block}>
            ...
            {selectedImage ? (
              <View style={styles.previewWrap}>
                ...
                <View style={styles.imageBadge}><Text style={styles.imageBadgeText}>PROOF</Text></View>
                <PrimaryButton onPress={removeSelectedImage} variant="quiet">{'删除当前照片'}</PrimaryButton>
              </View>
            ) : (
            ...
            <View style={styles.photoActions}>
              <View style={styles.photoActionItem}><PrimaryButton onPress={takePhoto}>{'拍照'}</PrimaryButton></View>
              <View style={styles.photoActionItem}><PrimaryButton onPress={pickImage} variant="soft">{'从相册选择'}</PrimaryButton></View>
            </View>
          </View>
...
function createStyles(theme: AppTheme) {
  return StyleSheet.create({
    ...
    heroMark: { position: 'absolute', right: -28, top: -36, width: 120, height: 120, borderRadius: 60, backgroundColor: theme.accent, opacity: 0.72 },
    kicker: { ...typography.caption, color: theme.primary, fontWeight: '900', marginBottom: spacing.sm },
    title: { ...typography.title, color: theme.text, fontWeight: '900', lineHeight: 36, marginBottom: spacing.sm, maxWidth: '86%' },
    criteria: { ...typography.body, color: theme.textMuted, lineHeight: 22 },
    focusText: { ...typography.caption, alignSelf: 'flex-start', overflow: 'hidden', borderRadius: radius.pill, backgroundColor: theme.surfaceAlt, color: theme.primary, fontWeight: '900', marginTop: spacing.md, paddingHorizontal: spacing.md, paddingVertical: spacing.xs },
    ...
    statusChip: { flex: 1, minHeight: 46 },
    journalInput: { minHeight: 120, backgroundColor: theme.surface },
    previewWrap: { overflow: 'hidden', borderColor: theme.border, borderRadius: radius.xlarge, borderWidth: 1, backgroundColor: theme.surfaceAlt, gap: spacing.sm, padding: spacing.sm },
    previewImage: { width: '100%', aspectRatio: 4 / 3, borderRadius: radius.large, backgroundColor: theme.surfaceAlt },
    imageBadge: { position: 'absolute', left: spacing.lg, top: spacing.lg, borderRadius: radius.pill, backgroundColor: theme.accent, paddingHorizontal: spacing.sm, paddingVertical: spacing.xs },
    imageBadgeText: { ...typography.micro, color: theme.text, fontWeight: '900' },
    photoEmpty: { alignItems: 'center', borderColor: theme.border, borderRadius: radius.xlarge, borderStyle: 'dashed', borderWidth: 1, backgroundColor: theme.surfaceAlt, padding: spacing.xl },
    photoDot: { width: 28, height: 28, borderRadius: 14, backgroundColor: theme.accent, marginBottom: spacing.sm },
    photoEmptyTitle: { ...typography.cardTitle, color: theme.text, fontWeight: '900', marginBottom: spacing.xs, textAlign: 'center' },
    photoEmptyText: { ...typography.body, color: theme.textMuted, lineHeight: 22, textAlign: 'center' },
