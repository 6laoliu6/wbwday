...
export default function ReviewByDateScreen() {
  ...
  const loadReview = useCallback(async () => {
    ...
    const [nextTasks, nextFocusSessions, nextProofs, nextReview] = await Promise.all([
      getTasksByDate(date),
      getFocusSessionsByDate(date),
      getCompletionProofsByDate(date),
  ...
  const stats = useMemo(
    () => buildReviewStats(tasks, focusSessions, proofs),
    [focusSessions, proofs, tasks],
  );
  ...
  const saveDailyReview = useCallback(async () => {
    ...
    try {
      const nextReview = await updateReview(date, {
        rating,
        summary,
        unfinishedTaskIdsMovedToTomorrow: movedTaskIds,
      });
      setReview(nextReview);
      void hapticSuccess();
      Alert.alert('今日复盘已保存。');
  ...
  const moveToTomorrow = useCallback(
    async (task: Task) => {
      if (!date || movedTaskIds.includes(task.id)) {
        return;
      }
  ...
  const selectRating = useCallback((nextRating: 1 | 2 | 3 | 4 | 5) => {
    ...
    return (
      <SafeAreaView style={styles.screen}>
        <View style={styles.empty}>
          <Text style={styles.emptyTitle}>没有找到这一天的复盘。</Text>
    ...
    <SafeAreaView style={styles.screen}>
      <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
        ...
        <View style={styles.overview}>
          {stats.totalCount === 0 ? (
            <Text style={styles.overviewMain}>今天还没有立愿记录。</Text>
          ) : (
            <>
              <Text style={styles.overviewEyebrow}>TODAY</Text>
              <Text style={styles.overviewMain}>
        ...
        <Section title="今日还愿照片墙" theme={theme}>
          <ProofPhotoWall proofs={stats.proofPhotos} />
        </Section>

        <Section title="完成任务" theme={theme}>
          {stats.completedTasks.length === 0 ? (
            <Text style={styles.mutedText}>今天还没有完全完成的任务。</Text>
          ) : (
            stats.completedTasks.map(({ task, proof }) => (
              <ReviewTaskItem
                key={task.id}
        ...
        <Section title="部分完成" theme={theme}>
          {stats.partialTasks.length === 0 ? (
            <Text style={styles.mutedText}>没有部分完成的任务。</Text>
          ) : (
            stats.partialTasks.map(({ task, proof }) => (
              <ReviewTaskItem
                key={task.id}
                onPress={() => router.push(`/task/${task.id}` as Href)}
                proof={proof}
        ...
        <Section title="明天可以继续" theme={theme}>
          {stats.unfinishedTasks.length === 0 ? (
            <Text style={styles.mutedText}>没有未完成任务。今天收得很干净。</Text>
          ) : (
            <>
              ...
              {stats.unfinishedTasks.map(({ task, proof }) => (
                <View key={task.id}>
                  {movedTaskIds.includes(task.id) ? (
                    <Text style={styles.movedText}>已移到明天</Text>
                  ) : null}
                  <ReviewTaskItem
                    onMarkPartial={() => markPartial(task)}
                    onMoveToTomorrow={() => moveToTomorrow(task)}
                    onPress={() => router.push(`/task/${task.id}` as Href)}
                    onSkip={() => Alert.alert('暂不处理', '没完成也没关系，明天可以继续。')}
                    proof={proof}
                    showUnfinishedActions={!movedTaskIds.includes(task.id)}
                    task={task}
                  />
                </View>
              ))}
        ...
        <Section title="今日评分" theme={theme}>
          <RatingSelector onChange={selectRating} value={rating} />
        </Section>
        ...
        <PrimaryButton disabled={saving} onPress={saveDailyReview} size="large">
          {saving ? '保存中' : review ? '更新今日复盘' : '保存今日复盘'}
        </PrimaryButton>
      </ScrollView>
    </SafeAreaView>
  );
...
function StatItem({ label, theme, value }: { label: string; theme: AppTheme; value: string }) {
  return (
    ...
    >
      <Text style={{ ...typography.section, color: theme.primary, fontWeight: '900' }}>{value}</Text>
      <Text style={{ ...typography.caption, color: theme.textMuted, fontWeight: '900', marginTop: spacing.xs }}>
        {label}
      </Text>
    </View>
  );
