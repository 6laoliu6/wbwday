...
function findCompletionStandard(text: string): string {
  ...
  for (const pattern of patterns) {
    ...
    if (match?.[0]) return cleanEdgePunctuation(match[0]);
  }
...
function estimateMinutes(text: string): number {
  ...
  if (minuteMatch?.[1]) return Math.max(1, Math.round(Number(minuteMatch[1])));

  const hourMatch = text.match(/(\d+(?:\.\d+)?)\s*小时/);
  if (hourMatch?.[1]) return Math.max(1, Math.round(Number(hourMatch[1]) * 60));

  return 30;
}

function removeCompletionStandardFromTitle(text: string, completionStandard: string): string {
  if (!completionStandard) return text;
...
function buildTitle(segment: string, completionStandard: string): string {
  ...
  return title.length >= MIN_SEGMENT_LENGTH ? title : cleanEdgePunctuation(withoutPrefix) || '未命名任务';
}
